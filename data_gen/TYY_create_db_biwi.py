...
def get_args():
	...
	parser.add_argument("--ad", type=float, default=0.2,
	                    help="enlarge margin")
			...
			if len(detected) > 0:
				...
				for i_d, d in enumerate(detected):
					...
					if d['confidence'] > 0.90:
						...
						#đảm bảo ROI ko vượt quá giới hạn ảnh 
						if cx - ds < 0:
							ds = cx
						...
						dis_betw_cen = np.sqrt(np.abs(cx- icx)**2 + np.abs(cy-icy)**2)
				...
				if dis_betw_frames < 80 or j==0:
					img = cv2.resize(img[yw1:yw2 , xw1:xw2, :], (img_size, img_size))
					...
					if np.abs(pitch) > 99 or np.abs(yaw) > 99 or np.abs(roll) > 99:
						continue
					out_imgs.append(img)
