...
class Bottleneck(nn.Module):
    def __init__(self, in_channels, out_channels, kernel, stride, exp_rate=2, activation=nn.ReLU, residual=False, groups=1, sort=False):
        ...
        self.exp_conv = nn.Sequential(
            conv_layer(in_channels, exp_channels, 1, 1, 0, bias=False, groups=groups),
            norm_layer(exp_channels),
            nlin_layer(inplace=True)
        )
        # dw-linear
        self.dw_conv = nn.Sequential(
            conv_layer(exp_channels, exp_channels, kernel, stride, padding, groups=exp_channels, bias=False),
            norm_layer(exp_channels),
            nlin_layer(inplace=True)
        )
        # pw-linear
        self.pw_conv = nn.Sequential(
            conv_layer(exp_channels, out_channels, 1, 1, 0, bias=False, groups=1),
...
# Mô hình chính sử dụng Network
class Model(nn.Module):
    def __init__(self, num_bins=66, M=99, cuda=False, bin_train=False, base=16, width_mult=1):
        super(Model, self).__init__()
        self.device = torch.device("cuda" if cuda and torch.cuda.is_available() else "cpu")
        self.network = Network(num_bins=num_bins, M=M, cuda=cuda, bin_train=bin_train, base=base, width_mult=width_mult).to(self.device)
...
if __name__ == '__main__':
    width_mult = 1
    model = Model(cuda=torch.cuda.is_available(), bin_train=False, num_bins=66)
