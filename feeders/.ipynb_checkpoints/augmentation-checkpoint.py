...
def load_occluders(pascal_voc_root_path):
    ...
    for annotation_path in annotation_paths:
        ...
        for i_obj, (xmin, ymin, xmax, ymax) in boxes:
            ...
            object_with_mask = object_with_mask[:,:,(2,1,0,3)]
...
def occlude_with_objects(im, occluders):
    ...
    count = np.random.choice([0,1,2,3,4], p=(0.2,0.3,0.3,0.2))
    try:
        for _ in range(count):
            occluder = random.choice(occluders)
           

            random_scale_factor = np.random.uniform(0.1,0.5)
            scale_factor = random_scale_factor * im_scale_factor
        
            occluder = resize_occluder(occluder)
...
def paste_over(im_src, im_dst, center):
    ...
    # alpha = np.expand_dims(np.squeeze(alpha,axis=-1),axis=-1)
    # print(alpha.shape,color_src.shape, region_dst.shape)
...
def resize_occluder(im):
    ...
    if h > w:
        new_size = tuple(np.round(np.array([s, w/h*s ])).astype(int))
    else: 
        new_size = tuple(np.round(np.array([h/w*s, s ])).astype(int))
    interp = cv2.INTER_AREA
    
    return cv2.resize(im, new_size, interpolation=interp)
