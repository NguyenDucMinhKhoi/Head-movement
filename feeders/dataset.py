...
#đọc các tệp .npz từ một thư mục và trả về một mảng numpy chứa tất cả các imgs và poses 
...
#Chuyển đổi góc Euler (yaw, pitch, roll) sang ma trận xoay 3x3 
def euler_to_mat(yaw, pitch, roll):
    ...
    return R
#Mở rộng và mã hóa các góc Euler thành một vector lớn hơn, sử dụng các hàm sin và cos
...
class Pose_300W_LP(Dataset):
    ...
    def __init__(self, data_dir, filename_path, ref_dir, ref_filename_path, transform=None):
        ...
        self.transform =  transforms.Compose([transforms.ToTensor(),transforms.RandomResizedCrop(size=(96,96),scale=(0.8,1.2)), transforms.ColorJitter(brightness=(0.8,1.2),contrast=(0.8,1.2),hue=(-0.2,0.2),saturation=(0.8,1.2)), transforms.Normalize(0,1)])
        ...
        self.cut_out = Cutout() #cắt ngẫu nhiên 
    ...
    def __getitem__(self, index):
        ...
        k = 15  #reference image 
        if self.data_dir == self.ref_dir:
            ref_idx = random.sample([i for i in range(0,self.length) if i != index], k)# img ref sẽ đc lấy cùng vs dataset đầu vào nhưng khác index
        else:
            ref_idx = random.sample(range(0,self.ref_length), k) #img ref đc lấy ngẫu nhiên, và cũng đc chuyển đổi và cắt ngẫu nhiên như input img 
        ref = []
        for i in range(k):
...
#phép biến đổi hình ảnh, mà cụ thể là làm mờ (mask out) ngẫu nhiên một số vùng hình chữ nhật trong hình ảnh.
class Cutout(object):
