...
class Processor():
    ...
    def load_model(self):
        ...
        if self.arg.weights:
            ...
            # self.print_log('Load weights from {}.'.format(self.arg.weight2))
       
            weights = torch.load(self.arg.weights)
            # weights_s = torch.load(self.arg.weight2)
            weights = OrderedDict(
                [[k.split('module.')[-1],
                  v.cuda(output_device)] for k, v in weights.items()])
            # weights_s = OrderedDict(
                # [[k.split('module.')[-1],
                #   v.cuda(output_device)] for k, v in weights_s.items()])
    ...
    def train(self, epoch, save_model=False):
        ...
        for batch_idx, ( img, ref, matrix_label, euler_label, ref_matrix_label,  ref_euler_label, index) in enumerate(process):
            total += img[0].size(0)
            img = [Variable(im.float().cuda(self.output_device), requires_grad=False) for im in img]
            ...
            for i in range(k):
                for j in range(9):
                    ref[i][j] = Variable(ref[i][j].float().cuda(self.output_device), requires_grad=False)
            ...
            # forward
            m, p,  me, rm,  rp, rme, d, reverse_d= self.model(img, ref, ref_matrix_label, x_pose=matrix_label, k=k,mode='train')
            pre_matrix = m[0]
            o_loss = 0
            pose_loss = 0 
            nll = 0
            ...
            #     for j in range(9):
            #         dis_loss += matrix_loss(ref_mat_dis[i][j],ref_matrix_label[i])
            #         dis_loss += matrix_loss(in_mat_dis[i][j], matrix_label)
            # dis_loss /= (2*9*len(ref))    

            d_loss = 0
            for i in range(len(ref)):
                for j in range(9):
                    d_loss += self.reg_criterion(d[i][j],d_label[i])
                    d_loss += self.reg_criterion(reverse_d[i][j], d_label[i])
            d_loss /= (18*len(ref))

            for i in range(9):
                o_loss += ortho_loss(me[i].view(-1,3,144//3))
                nll += nll_loss(p[i], matrix_label)
                pose_loss += matrix_loss(m[i], matrix_label)  
            
            for i in range(k):
                for j in range(9):
                    o_loss += ortho_loss(rme[i][j].view(-1,3,144//3))
                    nll += nll_loss(rp[i][j], ref_matrix_label[i])
            ...
            for i in range(k): 
                for j in range(9):
                    pose_loss += matrix_loss(rm[i][j], ref_matrix_label[i]) 
            
            # proj_loss = 0
            # for i in range(len(ref)):
            #     for j in range(9):
            #         proj_loss += matrix_loss(mem[i][j], matrix_label)
            #         proj_loss += matrix_loss(rmem[i][j], ref_matrix_label[i])
            # proj_loss /= (18*k)
            # Total loss
            ...
            # for i in range(k):
            #     for j in range(9):
            #         mat_dis_loss += self.reg_criterion(compute_distance(m[j], rm[i][j]), d_label[i])
            # mat_dis_loss /= k*9
            # d_feat_loss  = 0
            # for i in range(k):
            #     for j in range(9):
            #         d_feat_loss +=self.reg_criterion(d_feat[i][j],d_label[i])
            #         d_feat_loss += self.reg_criterion(reverse_d_feat[i][j],d_label[i])

            # d_feat_loss /= (18*k)

            loss = pose_loss/(9*k+9) + 1e-6*o_loss/(9*k+9) + 0.5*nll/(9*k+9) + 0.5*d_loss
...
def compute_distance(matrix1, matrix2):
    ...
    return d
def matrix_loss(matrix, matrix_label, ortho=True):
